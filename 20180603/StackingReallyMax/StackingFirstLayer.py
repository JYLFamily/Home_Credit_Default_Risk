...
class StackingMax(object):
    ...
    def model_fit(self):
        def __get_oof(clf, train_feature, train_label, test_feature):
            folds = StratifiedKFold(n_splits=2, shuffle=True, random_state=7)
        ...
        lb_oof_train, lb_oof_test = __get_oof(
            lb,
    ...
    def model_predict(self):
        self.__oof_train = pd.DataFrame(
            self.__oof_train,
            columns=["cb", "lb", "xg_tree", "xg_linear", "et_gini", "et_entropy", "rf_gini", "rf_entropy"]
        )
